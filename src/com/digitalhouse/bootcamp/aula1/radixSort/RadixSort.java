...
public class RadixSort {
	...
	private final int INITIAL_ARRAY[] = { 16223, 898, 13, 906, 235, 23, 9, 1532, 6388, 2511, 8 };
	...
	public void radixSort() {
		...
		String str = Arrays.toString(INITIAL_ARRAY).replaceAll("\\s+", "");
	...
	public void fillHashMap(HashMap<Integer, Integer> values, String[] iArrStr, int actualPosition) {
		for (String elem : iArrStr) {
			...
			if (!(elem.length() < actualPosition)) {
				digit = Integer
						.parseInt(elem.substring(elem.length() - actualPosition, elem.length() - actualPosition + 1));
			}
